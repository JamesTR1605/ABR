...
#include <string>          // Librería para manipular cadenas de texto (string)
...
struct Persona {
    ...
    // Punteros para árbol binario de búsqueda
    ...
    Persona* hijos[10];            // Arreglo de punteros a hijos (máximo 10)
...
private:
    ...
    // Encuentra el nodo con valor mínimo (para eliminar)
...
// Llama a la función recursiva que busca a la persona dentro del árbol genealógico
...
void eliminarPersona(int id) {
    // Validación: no se permite eliminar ni a Dios (ID 0) ni a los abuelos fundadores (ID 1–4)
    if (id >= 0 && id <= 4) {
        cout << "Error: No se pueden eliminar la entidad suprema o los Abuelo fundadores ." << endl;
        return;
    }
    ...
    if (persona != NULL) {
        ...
        // Llama a la función recursiva para eliminar al nodo del árbol
...
// Mostrar información detallada de una persona según su ID
void mostrarInfoPersona(int id) {
    ...
    if (persona != NULL) {
        ...
        // Mostrar información de los padres si existen
...
// Recorre el árbol en inorden y muestra a cada persona como posible padre/madre
...
// Mostrar recorrido postorden (izquierda, derecha, raíz)
void mostrarPostorden() {
    cout << "\n=== ÁRBOL GENEALOGICO (POSTORDEN) ===" << endl;
    postordenRecursivo(raiz);
}

// Mostrar árbol por niveles (BFS - Breadth First Search)
...
// Mostrar árbol por generaciones
...
// Función recursiva para mostrar descendientes (ID > 4)
...
int main() {
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 5: {
                int id;
                cout << "Ingrese ID para mostrar información: ";
                cin >> id;
